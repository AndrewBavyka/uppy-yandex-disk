...
function adaptData(res, userInfo) {
  const data = {
...
export default class YandexDisk {
  ...
  async list({ token, directory, limit = 20, offset = 0 }) {
    const directoryPath = Array.isArray(directory) ? directory[0] : directory;

    console.log(directoryPath);
   
    const path = directoryPath
      ? `?path=${encodeURIComponent(directoryPath)}&limit=${limit}&offset=${offset}`
    ...
    return adaptData(data, userInfo);
  }
